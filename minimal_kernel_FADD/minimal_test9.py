...
# Optional: NVTX labels (commented out below)
...
def main():
    ...
    # --- Warm-up (JIT) so events don't include compilation ---
    ...
    # CUDA events
    evt_start_A = torch.cuda.Event(enable_timing=True)
    evt_end_A   = torch.cuda.Event(enable_timing=True)

    # nvtx.range_push("globaltimer_kernel")  # optional label
    evt_start_A.record()
    debug_timer_globaltimer_kernel[grid](
        xA, t0, t1, tick_ns,
    ...
    elapsed_ms_A = evt_start_A.elapsed_time(evt_end_A)  # ms
    elapsed_us_A = elapsed_ms_A * 1e3
    ...
    # CUDA Events summary for kernel A
    print("CUDA Events (kernel A):")
    print(f"  elapsed (device) = {elapsed_us_A:.3f} µs  ({elapsed_ms_A:.6f} ms)")
    print(f"  per-iteration (approx) = {(elapsed_us_A*1e3)/REPS:.2f} ns/add\n")
    ...
    # Warm-up to avoid JIT time in events
    ...
    evt_start_B = torch.cuda.Event(enable_timing=True)
    evt_end_B   = torch.cuda.Event(enable_timing=True)

    # nvtx.range_push("clock64_kernel")
    evt_start_B.record()
    debug_timer_clock64_kernel[grid](
        xB, c0, c1, tick_cyc,
    ...
    elapsed_ms_B = evt_start_B.elapsed_time(evt_end_B)
    elapsed_us_B = elapsed_ms_B * 1e3
    ...
    print(f"  mean per iteration: {mean_dc / REPS_CLOCK64:.4f} cyc/add")
    print(f"  (assuming {GHz:.3f} GHz) ⇒ {((mean_dc / REPS_CLOCK64) * ns_per_cycle):.4f} ns/add")

    # CUDA Events summary for kernel B
    print("CUDA Events (kernel B):")
    print(f"  elapsed (device) = {elapsed_us_B:.3f} µs  ({elapsed_ms_B:.6f} ms)")
    print(f"  per-iteration (approx) = {(elapsed_us_B*1e3)/REPS_CLOCK64:.2f} ns/add\n")
