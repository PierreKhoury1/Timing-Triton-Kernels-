...
def debug_timer_globaltimer_kernel(
    ...
    x = tl.load(x_ptr + offs, mask=m, other=0).to(tl.uint32)
    y = tl.full([BLOCK], 0xAAAAAAAA, tl.uint32)
    z = tl.full([BLOCK], 0x55555555, tl.uint32)
    t0 = _read_globaltimer()
    for _ in tl.static_range(REPS):
        # lop3.b32 dst, a, b, c, 0xCA ;  (for example: (a & b) | (~a & c))
        x = tl.inline_asm_elementwise(
            "{ lop3.b32 $0, $1, $2, $3, 0xCA; }",
            constraints="=r,r,r,r",
            args=[x, y, z],
            dtype=tl.uint32,
            is_pure=False,
