...
def debug_timer_globaltimer_kernel(
    ...
    #this is to stimulate x =  x  * 17  + 5
    mul_c = tl.full([BLOCK], 17, tl.int32)
    add_c = tl.full([BLOCK], 5, tl.int32)
    t0 = _read_globaltimer()
    for _ in tl.static_range(REPS):
        # PTX-ish inline asm: mad.lo.s32 dst, a, b, c   --> dst = a*b + c
        x = tl.inline_asm_elementwise(
            "{ mad.lo.s32 $0, $1, $2, $3; }",
            constraints="=r,r,r,r",
            args=[x, mul_c, add_c],
            dtype=tl.int32,
            is_pure=False,   # tell Triton not to move/remove it
            pack=1,
        )
