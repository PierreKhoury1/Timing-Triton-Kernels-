...
def debug_timer_globaltimer_kernel(
    ...
    # 2) arithmetic region: dependent f16x2 FMA
    # x_ptr should point to 32-bit values that actually hold 2x fp16
    packed = tl.load(x_ptr + offs, mask=m, other=0).to(tl.uint32)

    # packed half(1.0) = 0x3C00 -> 0x3C003C00
    mul_packed = tl.full([BLOCK], 0x3C003C00, tl.uint32)
    # packed half(0.5) = 0x3800 -> 0x38003800
    add_packed = tl.full([BLOCK], 0x38003800, tl.uint32)
    ...
    for _ in tl.static_range(REPS):
        ...
        packed = tl.inline_asm_elementwise(
            ...
            constraints="=r,r,r,r",
            args=[packed, mul_packed, add_packed],
            dtype=tl.uint32,
            is_pure=False,
            pack=1,
        )
    t1 = _read_globaltimer()
    tl.store(x_ptr + offs, packed, mask=m)
